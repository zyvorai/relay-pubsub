...
Publish → pull → ack against the real gateway. Generate Demo catalog first if the topic is missing.

## When to use it

- Open **Live demo** when the job matches this screen
- Prefer the product home / Get started panel if you are unsure where to begin
- Confirm health and auth tokens if probes fail

## How to get there

- UI path: `/ui/` → **Live demo** (or matching nav tab)
...
3. Complete the on-screen fields / actions for this surface (Publish → pull → ack against the real gateway. Generate Demo catalog first if the topic is missing.).
