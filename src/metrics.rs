...
impl Metrics {
    pub fn new() -> Self {
        ...
        let requests = IntCounterVec::new(
            Opts::new("relay_pubsub_requests_total", "Gateway requests by operation and result"),
            &["transport", "operation", "result"],
        ).expect("valid request metric");
        let messages = IntCounterVec::new(
            Opts::new("relay_pubsub_messages_total", "Messages published or delivered"),
            &["direction"],
        ).expect("valid message metric");
        let latency = HistogramVec::new(
            HistogramOpts::new("relay_pubsub_request_duration_seconds", "Gateway request latency"),
            &["transport", "operation"],
        ).expect("valid latency metric");
        registry.register(Box::new(requests.clone())).expect("register requests");
        registry.register(Box::new(messages.clone())).expect("register messages");
        registry.register(Box::new(latency.clone())).expect("register latency");
        Self { registry, requests, messages, latency }
    }
