...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gateway = GatewayService::new(backend.clone(), config.gateway_auth_token.clone(), metrics.clone());
    let http_state = HttpState { backend, auth_token: config.gateway_auth_token.clone(), metrics: metrics.clone() };
